...
SRC_URI="http://www.gaia-gis.it/spatialite-${PV}-4/${MY_P}.tar.gz"
...
RDEPEND="geos? ( >=sci-libs/geos-3.1 )
        proj? ( sci-libs/proj )
...
src_configure()
{
    epatch "${FILESDIR}"/${PN}-2.3.1-quiet.patch
...
pkg_postinst() {
    ...
    elog "you can emerge spatialite-tools, spatialite-gui and spatialite-gis afterwards:"
    elog
    elog "emerge -av spatialite-tools"
    elog "emerge -av spatialite-gui"
    elog "emerge -av spatialite-gis"
    elog
}
