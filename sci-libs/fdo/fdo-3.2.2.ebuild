...
src_compile() {
    ...
    sed -i -e "s:mkdir -p \"/usr/local/fdo-3.2.0/lib\":mkdir -p \"${D}/usr/local/fdo-3.2.2/lib\":" \
        setenvironment.sh || die "bad sed"

    sed -i -e "s:export LD_LIBRARY_PATH=\$LD_LIBRARY_PATH\:/usr/local/fdo-3.2.0/lib\:\$SDEHOME/lib:export LD_LIBRARY_PATH=\$LD_LIBRARY_PATH\:/usr/local/fdo-3.2.2/lib\:\$SDEHOME/lib:" \
        setenvironment.sh || die "bad sed"
