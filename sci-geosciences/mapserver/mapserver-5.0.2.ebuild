...
IUSE="agg doc fastcgi flash gdal geos java occi pdf perl php postgis proj python ruby tcl threads tiff unicode xml xpm" # mono
...
RDEPEND="media-libs/libpng
	...
	occi? ( >=dev-db/oracle-instantclient-basic-10.2.0.3 )
...
src_compile() {
    ...
    if use occi && -z "$ORACLE_HOME"; then
        die "You have compiled with occi use flag but your ORACLE_HOME" \
            "points to nirvana"
    else
        myconf="${myconf} --with-oraclespatial=$ORACLE_HOME"
    fi
