...
inherit eutils rpm check-reqs multilib
...
RDEPEND="sys-devel/bc
	x86? ( >=dev-libs/libaio-0.3.96 )
	amd64? ( >=app-emulation/emul-linux-x86-libaio-0.3.96 )
...
pkg_config() {
	...
	if [ "$change" != "no" ] || [ "$changeport" != "no" ]; then
		...
		elog "Then re-run this ebuild .. config"
	else
		elog "Kernel parameters set, configure oracle-xe using"
		elog "  ${ORACLEHOME}/bin/oracle_configure.sh"
		elog "Make sure to run \"env-update && source /etc/profile\" if you want"
		elog "to run the config script from the current shell."
	fi
}
